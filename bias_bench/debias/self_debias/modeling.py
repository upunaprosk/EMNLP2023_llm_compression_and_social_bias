...
class GenerativeLMWrapper(ABC):
    ...
    def get_token_probability_distribution(
        ...
        kwargs = {"add_prefix_space": True} if isinstance(self, LLamaWrapper) else {}
        kwargs = {"add_prefix_space": True} if isinstance(self, OPTWrapper) else {}
...
class LLamaWrapper(GenerativeLMWrapper):
    ...
    def __init__(self, model_name: str = "/home/yena.seo/ai_projects/models/llama_hf/7B", use_cuda: bool = True):
        """
        :param model_name: the name of the pretrained llama model (default: "gpt2-xl")
        :param use_cuda: whether to use CUDA
        """
        super().__init__(use_cuda=use_cuda)
        self._tokenizer = LlamaTokenizer.from_pretrained(model_name, device_map="auto")
        self._model = SelfDebiasingLLamaLMHeadModel.from_pretrained(model_name, device_map="auto")
        # if use_cuda:
        #     self._model.parallelize()
...
class OPTWrapper(GenerativeLMWrapper):

    def __init__(self, model_name: str = "facebook/opt-125m", use_cuda: bool = True):
        """
        :param model_name: the name of the pretrained llama model (default: "gpt2-xl")
        :param use_cuda: whether to use CUDA
        """
        super().__init__(use_cuda=use_cuda)
        self._tokenizer = AutoTokenizer.from_pretrained(model_name, device_map="auto")
        self._model = SelfDebiasingOPTLMHeadModel.from_pretrained(model_name, device_map="auto")
        # if use_cuda:
        #     self._model.parallelize()
        self._tokenizer.pad_token = self._tokenizer.eos_token
        self._model.config.pad_token_id = self._tokenizer.eos_token_id
    ...
    def generate_self_debiasing(self, input_texts: List[str], debiasing_prefixes: List[str], decay_constant: float = 50,
                                epsilon: float = 0.01, debug: bool = False, min_length: int = None,
                                max_length: int = None,
                                **kwargs) -> List[str]:

        self._model.init_logits_processor(num_debiasing_prefixes=len(debiasing_prefixes), decay_constant=decay_constant,
                                          epsilon=epsilon,
                                          debug=debug, tokenizer=self._tokenizer)
        inputs = input_texts.copy()
        for debiasing_prefix in debiasing_prefixes:
    ...
    def compute_loss_self_debiasing(self, input_ids: torch.Tensor, trg_len: int, debiasing_prefixes: List[str],
                                    decay_constant: float = 50,
                                    epsilon: float = 0.01, debug: bool = False) -> torch.Tensor:
        ...
        shifts = input_prefixes['attention_mask'].shape[-1] - input_prefixes['attention_mask'].sum(dim=-1)
        for batch_idx in range(input_prefixes['input_ids'].shape[0]):
            input_prefixes['input_ids'][batch_idx] = input_prefixes['input_ids'][batch_idx].roll(
        ...
        outputs = self._model(input_ids=input_ids_repeated, attention_mask=attention_mask, position_ids=position_ids,
                              labels=target_ids)
        lm_logits = outputs[1]
